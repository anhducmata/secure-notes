...
- `SecureNotesAppleCore` Swift package with:
  ...
  - Soniox WebSocket client for streaming diarized transcription
  - Apple on-device Speech framework (`SFSpeechRecognizer`) for offline transcription (defaults to "Other" when diarization is unavailable)
  - Native system audio capture via `ScreenCaptureKit` with application feedback exclusion
...
- Info.plist keys:
  - `NSMicrophoneUsageDescription`: Permission to access microphone
  - `NSSpeechRecognitionUsageDescription`: Permission for on-device speech transcription
  - `BGTaskSchedulerPermittedIdentifiers`: `com.mata.securenotes.transcription`
